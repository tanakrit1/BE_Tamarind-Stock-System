...
export const HandleErrorException = (error: {
  ...
  if (error instanceof UnauthorizedException) {
    ...
    if (message?.includes('No column name was specified for column')) {
      const regex = /column (\d+) of '([^']+)'/.exec(message);
      if (regex) {
        const columnName = regex[1];
        const tableName = regex[2];
        return new BadRequestException(
          `${columnName} of table ${tableName}`,
          `ไม่พบชื่อบางคอลัมน์ใน ${columnName} ตาราง ${tableName} ที่คุณต้องการ กรุณาตรวจสอบคอลัมน์และลองอีกครั้ง`,
        );
      } else {
        return new InternalServerErrorException();
      }
